...
const Wirte = (props) =>{
    ...
    const write_CallML = async () =>{
        const token = getAccessToken();
        try{
            const wRes = await axios.post(process.env.REACT_APP_SERVER_ROOT_URL+"/board/upload",{
                "content" : content,
                "title" : title
               },
               {
               ...
               const recc = await axios.post(process.env.REACT_APP_SERVER_ROOT_URL+"/recommend",{
                "members":[
